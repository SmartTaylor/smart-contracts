...
contract("Simulation", async (accounts) => {
  ...
  it("Buyers got correct amount of tokens", async () => {
    for(var i = 1; i < accounts.length; i++){
      ...
      assert.equal(((balance.toNumber() + 1) / (simulation.tokenBalances[i] + 1)).toFixed(8),1.00000000000000 );
    }
  })
  ...
  it("Sale raised the correct amount of money", async () => {
    ...
    assert.equal((raised / (walletBal.toNumber() - walletBalance.toNumber())).toFixed(8),1.0000000000000);
  })

  it("accounts correctly for tokens", async () => {
    const sold = await sale.tokensSold();
    assert.equal(simulation.tokensSold, sold.toNumber());
  })
})
